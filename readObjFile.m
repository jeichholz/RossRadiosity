function [vertexList,faceList,colorList,emissionList,objectMap]=readObjFile(objfile, silent)
%function [vertexList,faceList,colorList,emissionList,objectMap]=readObjFile(objfile, silent)
%
...
%OUTPUT -
%
%vertexList - N x 3 list of vertices.  In the form vertexList=[v1x, v1y, v1z;
%                                                              v2x, v2y, v2z;
%                                                              v3x, v3y, v3z;
%                                                               .   .      .;
%                                                              vNx, vNy, vNz];
%
%faceList - an M x 3 list of faces. Face i is the triangle connecting vertices vi1, vi2, vi3.
%           so if F(5,:) is [8, 13, 109] then face 5 is a triangle whose vertex coordinates can be found in
%           V(8,:), V(13,:), and V(109,:).
%
%
%colorList - an M x 3 list of reflectances for each face.  If colorList(5,:) = [0.2, 0.9, 0.4]
...
%
%emissionList - an M x 3 list of emissions.  If emissionList(5,:) = [4,9,230] then face 5 is emitting 4/sec red, 9/sec green, and 230/sec blue light.
%
...
faceColorHasBeenDefinedByMaterial=[];
    ...
    if ~isempty(line)
        ...
        switch parts{1}
            ...
            case 'mtllib'
                ...
                mtlLib=parseMaterialLibrary(materialLibraryFile);
            case 'usemtl'
            ...
            case 'v'
                ...
                vertexList(vertexCount,:)=[str2double(parts{2}), str2double(parts{3}), str2double(parts{4})];
            ...
            case 'f'  %Specifying a face, gotta figure out exactly how they are specifying
                ...
                if size(vtx_spec,1)>3  %This is not a triangle!
                    ...
                    vertex_coordinates=vertexList(vtx_spec(:,1),:)';  %Coordinates of the vertices in this polygon.
                   ...
                   % vertex_coordinates=HouseholderReflector(vertex_coordinates(:,2))*vertex_coordinates; %Rotate out the 2nd column (first is already at origin) to be [1;0;0]
                    ...
                    original_normal=cross(vertexList(vtx_spec(2,1),:)-vertexList(vtx_spec(1,1),:), vertexList(vtx_spec(3,1),:)-vertexList(vtx_spec(1,1),:));

                    facesToAdd=[];
                    for i=1:size(dt,1)
                        triangle_normal=cross(vertexList(vtx_spec(dt(i,2),1),:)-vertexList(vtx_spec(dt(i,1),1),:), vertexList(vtx_spec(dt(i,3),1),:)-vertexList(vtx_spec(dt(i,1),1),:));
                ...
                for i=1:size(facesToAdd,1)
                    ...
                    if length(vertexColorIsNonDefault)>= max(facesToAdd(i,:)) && all(vertexColorIsNonDefault(facesToAdd(i,:)))
                        colorList(end+1,:)=mean(vertexColorList(facesToAdd(i,:),:));
                        facesWVertexColor=facesWVertexColor+1;
                    else
                        try
                            colorList(end+1,:)=mtlLib(currentMaterial);
                        catch e
                            e;
                        end
                    ...
                    try
                    objectMap(currentObject)=[objectMap(currentObject), size(faceList,1)];
                    catch e
                        e;
                    end
                    if lightingOn
                        emissionList(end+1,:)=colorList(end,:)*lightingPower;
                        litFaces=litFaces+1;
                    else
                        emissionList(end+1,:)=[0,0,0];
                    end
...
if ~silent
    ...
    function lib=parseMaterialLibrary(fname)
